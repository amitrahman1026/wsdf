...
extern "C" fn plugin_register() {
    ...
    unsafe {
        plug.register_protoinfo = std::option::Option::Some(
            <BabyUDP as wsdf::Protocol>::proto_register,
        );
        plug.register_handoff = std::option::Option::Some(
    ...
    fn subdissector_map(
        ...
        const SUBDISSECTORS: ::std::thread::LocalKey<wsdf::SubdissectorMap> = {
            ...
            unsafe {
                ...
                LocalKey::new(const {
                    if needs_drop::<wsdf::SubdissectorMap>() {
                        |init| {
                            #[thread_local]
                            static VAL: LazyStorage<wsdf::SubdissectorMap, ()> = LazyStorage::new();
    ...
    fn hf_map(op: wsdf::HfMapOp) -> std::option::Option<std::ffi::c_int> {
        const HFS: ::std::thread::LocalKey<wsdf::HfMap> = {
            ...
            unsafe {
                ...
                use ::std::thread::local_impl::LazyStorage;
                LocalKey::new(const {
                    if needs_drop::<wsdf::HfMap>() {
                        |init| {
                            #[thread_local]
                            static VAL: LazyStorage<wsdf::HfMap, ()> = LazyStorage::new();
                            VAL.get_or_init(init, __init)
                        }
                    } else {
                        |init| {
